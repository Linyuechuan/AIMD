...
fprintf(ID,'E_a(eV)=%f¡À%.1G\r\nD_0(cm^2/s)=%f¡À%.1G\r\nD_300(cm^2/s)=%f¡À%.1G\r\nConductivity(mS/cm)=%f¡À%.1G',E_a/(1.602*10^(-19)),u_E_a/(1.602*10^(-19)),D_0*10^4,u_D_0*10^4,D_300*10^4,u_D_300*10^4,Conductivity*10,u_Conductivity*10);
fclose(ID);
% H=[sum(u.^-2),sum(u.^-2.*x);sum(u.^-2.*x),sum(u.^-2.*x.^2)];
% b=[sum(u.^-2.*y);sum(u.^-2.*x.*y)];
% C=H^-1*b
